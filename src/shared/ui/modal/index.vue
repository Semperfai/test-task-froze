<template>
  <v-dialog v-model="dialogComputed" max-width="800">
    <v-card>
      ...
      <v-card-actions>
        <v-btn class="close-btn" @click="closeModal">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
import { type IPackaItemDetails } from "../../types";
const props = defineProps<{
  selectedItem?: IPackaItemDetails;
  dialog: boolean;
}>();
const { dialog } = toRefs(props);
const emit = defineEmits(["update:dialog"]);
const dialogComputed = computed(() => dialog.value);
